...
var cf = {};
	cf.start = function(){
		...
		function slide( active ){
			...
			var normal = $(".cf-before")[0] ? parseInt( $($(".cf-before")[0]).width() ) : parseInt( $($(".cf-after")[0]).width() );
			var margin = $(".cf-before")[0] ? parseInt( $($(".cf-before")[0]).css("margin-right") ) : parseInt( $($(".cf-after")[0]).css("margin-left") );
			margin = margin * 0.85;
			if(margin < 0){
				margin = margin * (-1);
			}
